...
uint8_t BMI088_Init(void)
{
    ...
    IIC_Write_One_Byte(&bmi088_bus, BMI088_GYRO_ADDR, GYRO_RANGE, 0x00);      // ±2000dps
...
void BMI088_Calibrate(void)
{
    ...
    // 绕X轴旋转180度安装分析：
    // 原始传感器：水平放置时，重力在+Z方向（+5461 LSB）
    // 绕X轴旋转180度后：Y变-Y，Z变-Z
    // 所以水平放置时，重力应该在-Z方向（-5461 LSB）
    
    // 但是！在应用坐标系转换后，我们会执行：acc->z = -raw_z
    // 所以校准后的raw_z应该是：测量值 - 偏移量
    // 我们期望转换后的z为+1g，即+5461 LSB
    // 所以：-raw_z = +5461  => raw_z = -5461
    // 因此偏移量应该是：测量值 - (-5461) = 测量值 + 5461
    
    acc_offset.x = (int16_t)(acc_sum_x / sample_count);
    acc_offset.y = (int16_t)(acc_sum_y / sample_count);
    acc_offset.z = (int16_t)(acc_sum_z / sample_count + 5461); 
